ScriptName Computer:BIOS extends Computer:Holotape
import Computer:Text
...
Event OnStartup(bool a)
	Debug.TraceSelf(self, "OnStartup", "argument:"+a)
	PrintLine("The shell is starting...")
	UI.Load(FlashMenu, "root1", OS_Asset, self, "BootCallback")
EndEvent


Event OnShutdown()
	Debug.TraceSelf(self, "OnShutdown", "Shutdown")
	Continue = false
EndEvent
...
Function BootCallback(bool success, string menuName, string menuRoot, string assetInstance, string assetFile)
	...
	While (Processing)
		...
		string line = ReadLine() ; record this for command history
...
Function Interpret(string line)
	...
	string[] command = Computer:Text.Split(line)

	Invoke(command) ; <--- testing for now
...
Function Invoke(string[] command)
	Debug.TraceSelf(self, "Invoke", command)
	UI.Invoke(FlashMenu, OS_Instance+".Invoke", ToArguments(command))
EndFunction


var[] Function ToArguments(string[] command)
	var[] arguments = new var[0]

	int index = 0
	While (index < command.Length)
		var element = command[index]
...
string Function CD(string[] command = none)
	...
	string value = UI.Invoke(FlashMenu, OS_Instance+".CD", arguments)
...
Function DIR(string[] command)
	...
	var object = UI.Invoke(FlashMenu, OS_Instance+".DIR", arguments)
...
Function TYPE(string[] command)
	...
	string object = UI.Invoke(FlashMenu, OS_Instance+".TYPE", arguments)
	string[] values = Utility.VarToVarArray(object) as string[]
...
Function HELP(string[] command)
	...
	If (command.Length == 1)
		...
		PrintLine(COMMAND_TREE + "      Graphically displays the directory structure of a path.")
